...
struct segtree {
	...
	ll cal (int num, int i, int x, int lx, int rx) // 1 3 // 1 8
	{
		if (sums[x] < num || rx < i) return -1;
		if (rx == lx)
			return lx - 1;
		int m = (lx + rx) / 2;
		int acc = cal (num, i, 2 * x, lx, m);
		if (acc == -1)
			acc = cal (num, i, 2 * x + 1, m + 1, rx);
		return acc;
	}

	ll cal (int num, int i)
	{
		return cal(num, i, 1, 1, size);
	}
